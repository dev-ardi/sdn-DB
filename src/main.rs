...
impl<'schema> Instance<'schema> {
    fn get(&self, key: &str) -> Option<&str> {
        self.schema
            .values
            .get(key)
            .map(|index| self.value.get(*index).map(|r| r.as_str()))
            .flatten()
    }
    fn add_child(&mut self, child: Instance<'schema>) {
        // the schema for the child should live at least as long as the parent.
